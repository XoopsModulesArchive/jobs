...
include(XOOPS_ROOT_PATH."/header.php");
	...
	$result = $xoopsDB->query("select usid, photo FROM ".$xoopsDB->prefix("jobs_listing")." where lid=".mysql_real_escape_string($lid)."");
	...
	$ok = !isset($_REQUEST['ok'])? NULL : $_REQUEST['ok'];
		...
		if ( in_array($member_id, $comp_users) ) {
			if($ok==1) {
			    $xoopsDB->queryf("delete from ".$xoopsDB->prefix("jobs_listing")." where lid=".mysql_real_escape_string($lid)."");
