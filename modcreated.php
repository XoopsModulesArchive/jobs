...
if (!empty($_POST['submit'])) {
	...
	if ($xoopsModuleConfig['jobs_resume_options'] == 'dhtmltextarea' || $xoopsModuleConfig['jobs_form_options'] == 'dhtml') {
		$made_resume = $myts->displayTarea($made_resume,0,0,0,0,0);
	} else {
		$made_resume = $myts->displayTarea($made_resume, 1,1,1,1,1);
	}

    $xoopsDB->query("update ".$xoopsDB->prefix("jobs_created_resumes")." set made_resume='$made_resume' where lid=".mysql_real_escape_string($lid)."");
    ...
    $result = $xoopsDB->query("select res_lid, lid, made_resume, date, usid from ".$xoopsDB->prefix("jobs_created_resumes")." where lid=".mysql_real_escape_string($lid)."");
