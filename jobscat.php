...
//	$orderbyTrans = jobs_convertorderbytrans($orderby);
	...
	$countpremium=$xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("jobs_listing")." where  premium='1' AND valid='Yes' AND cid=".mysql_real_escape_string($cid)."$cat_perms");
	...
	$countresult=$xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("jobs_listing")." where valid='Yes' AND cid=".mysql_real_escape_string($cid)."$cat_perms");
		list($trow) = $xoopsDB->fetchRow($countresult);
		$trows = $trow;

	$result = $xoopsDB->query("select cid, pid, title from ".$xoopsDB->prefix("".$mydirname."_categories")." where cid=".mysql_real_escape_string($cid)." $cat_perms");
...
//  we need to examine $orderby and CAST alpha fields that we want to sort numerically. $orderby format is [field ASC] or [field DESC]
	...
	$sql="select lid, cid, title, status, expire, type, company, price, typeprice, date, town, state, valid, premium, photo, view from ".$xoopsDB->prefix("jobs_listing")." WHERE cid=".mysql_real_escape_string($cid)." AND valid='yes' AND status!='1' $cat_perms order by $orderby1";
