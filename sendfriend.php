...
function SendJob($lid=0)
{
	...
	$result = $xoopsDB->query("select lid, title, type FROM ".$xoopsDB->prefix("jobs_listing")." where lid=".mysql_real_escape_string($lid)."");
...
function MailJob($lid=0, $yname, $ymail, $fname, $fmail)
{
  ...
  if ($xoopsModuleConfig["jobs_use_captcha"] == '1') {
	...
	$result = $xoopsDB->query("select lid, title, type, company, desctext, requirements, tel, price, typeprice, contactinfo, date, email, submitter, town, state, photo FROM ".$xoopsDB->prefix("jobs_listing")." where lid=".mysql_real_escape_string($lid)."");
...
function SendResume($lid=0)
{
	...
	$result = $xoopsDB->query("select lid, name, title FROM ".$xoopsDB->prefix("jobs_resume")." where lid=".mysql_real_escape_string($lid)."");
...
function MailResume($lid=0, $yname, $ymail, $fname, $fmail)
{
  ...
  if ($xoopsModuleConfig["jobs_use_captcha"] == '1') {
	...
	$result = $xoopsDB->query("select lid, cid, name, title, exp, expire, private, tel, salary, typeprice, date, email, submitter, usid, town, state, valid, resume, rphoto, view FROM ".$xoopsDB->prefix("jobs_resume")." where lid=".mysql_real_escape_string($lid)."");
