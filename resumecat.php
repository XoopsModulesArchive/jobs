...
//   options are -  'dateD'   'dateA'  'titleA'   'titleD'   'viewA'   'viewD'   'townA'   'townD'   'stateA'   'stateD'
	...
	$pathstring = "<a href='index.php'>".$mydirname."</a> ".$backarrow." <a href='resumes.php'>"._JOBS_RES_LISTINGS."</a>";
	...
	$countresult=$xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("jobs_resume")." where valid='Yes' AND cid=".mysql_real_escape_string($cid)."$cat_perms");
		list($trow) = $xoopsDB->fetchRow($countresult);
		$trows = $trow;
	
	$result = $xoopsDB->query("select cid, pid, title from ".$xoopsDB->prefix("jobs_res_categories")." where  cid=".mysql_real_escape_string($cid)."$cat_perms");
...
//	$categories = resume_MygetItemIds("".$mydirname."_view");
	...
	if (in_array($sub_ele['cid'], $categories)) {
		...
		$infercategories .= "<a href=\"".XOOPS_URL."/modules/$mydirname/resumecat.php?cid=".$sub_ele['cid']."\">".$chtitle."</a>";
		...
		$xoopsTpl->append('subcategories', array('title' => $myts->htmlSpecialChars($ele['title']), 'id' => $ele['cid'], 'infercategories' => $infercategories, 'totallisting' => resume_getTotalResumes($ele['cid'], 1), 'count' => $scount, 'new' =>  "&nbsp;".resume_categorynewgraphic($ele['cid']).""));
		$scount++;
