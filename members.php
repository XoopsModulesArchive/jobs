...
// For Premium Users
	if ($istheirs == "1") {
	$countresult=$xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("".$mydirname."_listing")." l, ".$xoopsDB->prefix("jobs_companies")." c where  l.company = c.comp_name and c.comp_id = ".mysql_real_escape_string($comp_id)." AND valid='Yes'");
	list($trow) = $xoopsDB->fetchRow($countresult);

	$sql="select l.lid, l.cid, l.title, l.status, l.expire, l.type, l.company, l.price, l.typeprice, l.date, l.email, l.submitter, l.usid, l.town, l.state, l.valid, l.photo, l.view, c.comp_id, c.comp_name, c.comp_img, c.comp_usid, c.comp_user1, c.comp_user2  FROM ".$xoopsDB->prefix("jobs_listing")." l, ".$xoopsDB->prefix("jobs_companies")." c WHERE l.company = c.comp_name and c.comp_id = ".mysql_real_escape_string($comp_id)." and valid = 'Yes' ORDER BY $orderby";
//To show non-approved ads to premium users remove  and valid = 'Yes'

	$result=$xoopsDB->query($sql,$show,$min);

	} else {
// For Non-Premium Users
	$countresult=$xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("".$mydirname."_listing")." l, ".$xoopsDB->prefix("jobs_companies")." c where  l.company = c.comp_name and c.comp_id = ".mysql_real_escape_string($comp_id)." AND valid='Yes' AND status!='1'");
	list($trow) = $xoopsDB->fetchRow($countresult);

	$sql="select l.lid, l.cid, l.title, l.status, l.expire, l.type, l.company, l.price, l.typeprice, l.date, l.email, l.submitter, l.usid, l.town, l.state, l.valid, l.photo, l.view, c.comp_id, c.comp_name, c.comp_img, c.comp_usid, c.comp_user1, c.comp_user2  FROM ".$xoopsDB->prefix("jobs_listing")." l, ".$xoopsDB->prefix("jobs_companies")." c WHERE l.company = c.comp_name and c.comp_id = ".mysql_real_escape_string($comp_id)."  and valid = 'Yes' and status != '1' ORDER BY $orderby";
	$result=$xoopsDB->query($sql,$show,$min);
	}
	...
	if ($istheirs == "1") {
	$replycount=$xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("jobs_replies")." where lid=".mysql_real_escape_string($lid)."");
	list($rrow) = $xoopsDB->fetchRow($replycount);
	$rrows = $rrow;
	$xoopsTpl->assign('reply_count', $rrows);

	$result2=$xoopsDB->query("select r_lid, lid, date, submitter, message, email, r_usid FROM ".$xoopsDB->prefix("".$mydirname."_replies")." where lid =".mysql_real_escape_string($lid)."");
	...
	if ($xoopsUser) {
	if ($isadmin) {
	$adminlink = "<a href='admin/index.php?op=ModJob&amp;lid=".addslashes($lid)."'><img src='images/modif.gif' border=0 alt=\""._JOBS_MODADMIN."\" /></a>";
	...
	if ($istheirs == "1") {
	$modify_link = "<a href='modjob.php?lid=".addslashes($lid)."'><img src='images/modif.gif' border=0 alt='"._JOBS_MODADMIN."' /></a>";
	}
	}
