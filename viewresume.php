...
$mytree = new JobTree($xoopsDB->prefix("jobs_res_categories"),"cid","pid");
	...
	$result=$xoopsDB->query("select r.lid, r.cid, r.name, r.title, r.exp, r.expire, r.private, r.tel, r.salary, r.typeprice, r.date, r.email, r.submitter, r.usid, r.town, r.state, r.valid, r.resume, r.rphoto, r.view, c.res_lid, c.lid, c.made_resume, c.date, c.usid, p.cod_img, p.lid, p.uid_owner, p.url FROM ".$xoopsDB->prefix("jobs_resume")." r LEFT JOIN ".$xoopsDB->prefix("jobs_created_resumes")." c ON c.lid = r.lid  LEFT JOIN ".$xoopsDB->prefix("jobs_pictures")." p ON r.lid = p.lid WHERE r.lid = ".mysql_real_escape_string($lid)."");
	...
	$result1 = $xoopsDB->query("select cid from ".$xoopsDB->prefix("jobs_resume")." where  lid=".mysql_real_escape_string($lid)."");
	list($cid) = $xoopsDB->fetchRow($result1);

	$result2 = $xoopsDB->query("select cid, pid, title from ".$xoopsDB->prefix("jobs_res_categories")." where  cid=".mysql_real_escape_string($cid)."");
	...
	list($res) = $xoopsDB->fetchRow($xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("jobs_resume")." where valid='Yes' AND cid=".mysql_real_escape_string($cid).""));
	...
	if ($recordexist) {
		list($lid, $cid, $name, $title, $exp, $expire, $private, $tel, $salary, $typeprice, $date, $email, $submitter, $usid, $town, $state, $valid, $resume, $rphoto, $view, $res_lid, $lid, $made_resume, $rdate, $rusid, $cod_img, $pic_lid, $uid_owner, $url)=$xoopsDB->fetchRow($result);
		...
		if ($viewcount_judge == true ){
			$xoopsDB->queryF("UPDATE ".$xoopsDB->prefix("jobs_resume")." SET view=view+1 WHERE lid = ".mysql_real_escape_string($lid)."");
		}
	...
	$xoopsTpl->assign('not_yet_approved',"<font style=\"color:red;\">". _JOBS_NOT_APPROVED."</font>");
		...
		$imprD = "<a href=\"print.php?op=Rprint&amp;lid=".addslashes($lid)."\" target=\"_blank\"><img src=\"images/print.gif\" border=\"0\" alt=\""._JOBS_RPRINT."\" width=\"15\" height=\"11\" /></a>&nbsp;";
		...
		if ($xoopsUser) {
			...
			if ($usid == $calusern) {
				$xoopsTpl->assign('modify', "<a href=\"modresume.php?lid=".addslashes($lid)."\"><img src=\"images/modif.gif\" border=0 alt=\""._JOBS_RES_MODIFANN."\"></a>&nbsp;<a href=\"delresume.php?lid=".addslashes($lid)."\"><img src=\"images/del.gif\" border=0 alt=\""._JOBS_DEL_RESUME."\" /></a>");

				$xoopsTpl->assign('add_photos', "<a href=\"../$mydirname/view_photos.php?lid=".addslashes($lid)."&uid=".addslashes($usid)."\">"._JOBS_ADD_PHOTOS."</a>");

			}
			if ($xoopsUser->isAdmin()) {
				$xoopsTpl->assign('admin', "<a href=\"admin/modresume.php?lid=".addslashes($lid)."\"><img src=\"images/modif.gif\" border=0 alt=\""._JOBS_MODRESADMIN."\" /></a>");
			}
		}
	...
	$result4 = $xoopsDB->query("select title from ".$xoopsDB->prefix("jobs_res_categories")." where cid=".mysql_real_escape_string($cid)."");
	list($ctitle) = $xoopsDB->fetchRow($result4);
	
	$xoopsTpl->assign('link_main', "<a href=\"../$mydirname/resumes.php\">"._JOBS_MAIN."</a>");
	$xoopsTpl->assign('friend', "<a href=\"../$mydirname/sendfriend.php?op=SendResume&amp;lid=".addslashes($lid)."\"><img src=\"../$mydirname/images/friend.gif\" border=\"0\" alt=\""._JOBS_SENDTOFRIEND."\" width=\"15\" height=\"11\" /></a>");
