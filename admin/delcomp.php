...
$myts =& MyTextSanitizer::getInstance();
	...
	$result = $xoopsDB->query("select comp_name, comp_usid, comp_img FROM ".$xoopsDB->prefix("jobs_companies")." where comp_id=".mysql_real_escape_string($comp_id)."");
	list($comp_name, $comp_usid, $photo) = $xoopsDB->fetchRow($result);

	$result1 = $xoopsDB->query("select company, usid FROM ".$xoopsDB->prefix("jobs_listing")." where usid=".mysql_real_escape_string($comp_usid)."");
...
// Delete Company
	$xoopsDB->queryf("delete from ".$xoopsDB->prefix("jobs_companies")." where comp_id=".mysql_real_escape_string($comp_id)."");

// Delete all listing by Company
	if ($comp_name == $their_company) {
	$xoopsDB->queryf("delete from ".$xoopsDB->prefix("jobs_listing")." where usid=".mysql_real_escape_string($comp_usid)."");
	}
