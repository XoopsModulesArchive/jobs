...
function ModRegionS($rid=0, $pid=0,$name, $abbrev)
{
	...
	$xoopsDB->query("update ".$xoopsDB->prefix("".$mydirname."_region")." set pid='$pid', name='$name', abbrev='$abbrev' where rid=$rid");
...
function RegionDel($rid=0, $ok=0)
{
	...
	$result = $xoopsDB->query("select name, abbrev FROM ".$xoopsDB->prefix("jobs_region")." where rid=".mysql_real_escape_string($rid)."");
...
// Delete Region
	$xoopsDB->queryf("delete from ".$xoopsDB->prefix("jobs_region")." where rid=".mysql_real_escape_string($rid)."");
