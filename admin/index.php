...
function Index()
{
	...
	// Add Listing
	echo "<br /><br /><fieldset><legend style='font-weight: bold; color: #900;'>". _AM_JOBS_ADD_LINK . "</legend><a href=\"submitlisting.php\">". _AM_JOBS_ADD_LINK . "</a>";

	echo "</fieldset><br />"; 

	// Add Resume
	echo "<br /><br /><fieldset><legend style='font-weight: bold; color: #900;'>". _AM_JOBS_RES_ADD_LINK . "</legend><a href=\"addresume.php\">". _AM_JOBS_RES_ADD_LINK . "</a>";
	...
	$result = $xoopsDB->query("select comp_id, comp_name, comp_date_added from ".$xoopsDB->prefix("jobs_companies")." order by comp_name");
...
function IndexView($lid=0)
{
    ...
    $result = $xoopsDB->query("select lid, cid, title, status, expire, type, company, desctext, requirements, tel, price, typeprice, contactinfo, contactinfo1, contactinfo2, date, email, submitter, usid, town, state, premium, photo from ".$xoopsDB->prefix("jobs_listing")." WHERE valid='No' AND lid='$lid'");
    $numrows = $xoopsDB->getRowsNum($result);
    if ($numrows>0) {
...
function IndexResumeView($lid=0)
{
    ...
    $result = $xoopsDB->query("select lid, cid, title, status, exp, expire, private, tel, salary, typeprice, date, email, submitter, usid, town, state, valid, resume from ".$xoopsDB->prefix("jobs_resume")." WHERE valid='No' AND lid='$lid'");
    $numrows = $xoopsDB->getRowsNum($result);
    if ($numrows>0) {
		echo "<table width='100%' border='0' cellspacing='1' cellpadding='8'><tr class='bg4'><td valign='top'>\n";
...
function ModJob($lid=0)
{
	...
	$result = $xoopsDB->query("select lid, cid, title, status, expire, type, company, desctext, requirements, tel, price, typeprice, contactinfo, contactinfo1, contactinfo2, date, email, submitter, usid, town, state, valid, premium, photo from ".$xoopsDB->prefix("jobs_listing")." where lid=$lid");
	...
	echo "<tr><td class='outer'>"._AM_JOBS_STATUS1."</td><td class='odd'><input type=\"radio\" name=\"status\" value=\"0\"";
	if ($status == "0") {
	echo "checked";
		}
	echo ">"._AM_JOBS_ACTIVE."&nbsp;&nbsp; <input type=\"radio\" name=\"status\" value=\"1\""; 
...
#  function ModJobS
#####################################################
function ModJobS($lid, $cid, $title, $status, $expire, $type, $company, $desctext, $requirements, $tel, $price, $typeprice, $contactinfo, $contactinfo1, $contactinfo2, $date, $email, $submitter, $usid, $town, $state, $valid, $premium)
{
...
#####################################################
function ListingValid($lid, $cid, $title, $status, $expire, $type, $company, $desctext, $requirements, $tel, $price, $typeprice, $contactinfo, $contactinfo1, $contactinfo2, $date, $email, $submitter, $usid, $town, $state, $valid, $premium, $photo)
{
...
#####################################################
function ResumeValid($lid, $cat, $title, $status, $exp, $expire, $private, $tel, $salary, $typeprice, $date, $email, $submitter, $town, $state, $valid, $resume)
{
    ...
    $xoopsDB->query("update ".$xoopsDB->prefix("jobs_resume")." set cid='$cat', title='$title', status='$status', exp='$exp', expire='$expire', private='$private', tel='$tel', salary='$salary', typeprice='$typeprice', date='$date', email='$email', submitter='$submitter', town='$town', state='$state', valid='$valid', resume='$resume'  where lid=$lid");
