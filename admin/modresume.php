...
if (!empty($_POST['submit'])) {
	...
	if ($del_made_resume == 1) {
		$xoopsDB->query("delete from ".$xoopsDB->prefix("jobs_created_resumes")." where lid=".mysql_real_escape_string($lid)."");
		$resume_old = "";
	}
    ...
    $xoopsDB->query("update ".$xoopsDB->prefix("jobs_resume")." set cid='$cid', name='$name', title='$title', status='$status', exp='$exp', expire='$expire', private='$private', tel='$tel', salary='$salary', typeprice='$typeprice', date='$date', email='$email', submitter='$submitter', town='$town', state='$state', valid='$valid', resume='$resume_old' where lid=$lid");

	if ($make_resume) {
	redirect_header("../createresume.php?lid=".addslashes($lid)."",4,_AM_JOBS_RES_UP_PLUS);
	exit();
	} else {
	redirect_header("index.php",4,_AM_JOBS_RES_MOD);
		exit();
	  }
	...
	$result = $xoopsDB->query("select lid, cid, name, title, status, exp, expire, private, tel, salary, typeprice, date, email, submitter, usid, town, state, valid, resume from ".$xoopsDB->prefix("jobs_resume")." where lid=".mysql_real_escape_string($lid)."");
	...
	$options["0"]=_AM_JOBS_ACTIVE;
	$options["1"]=_AM_JOBS_INACTIVE;
