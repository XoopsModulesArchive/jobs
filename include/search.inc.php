...
if ($is_resume != 1) {
	...
	$sql = "SELECT  lid,cid,title,type,company,desctext,requirements,tel,price,contactinfo,town,state,usid,valid,date FROM ".$xoopsDB->prefix("jobs_listing")." WHERE valid='Yes'  AND date<=".time()."$cat_perms";
	...
	$sql = "SELECT lid, cid, name, title, exp, expire, private, salary, typeprice, date, usid, town, state, valid FROM ".$xoopsDB->prefix("jobs_resume")." WHERE valid='Yes' and date<=".time()." $rescat_perms";
