...
// shows how many replies to show default is 1
	...
	$countresult=$xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("jobs_replies")." where lid=".mysql_real_escape_string($lid)."");
	...
	$sql="select r_lid, lid, title, date, submitter, message, resume, tele, email, r_usid, company FROM ".$xoopsDB->prefix("jobs_replies")." WHERE lid=".mysql_real_escape_string($lid)." order by $orderby";
	...
	while(list($r_lid, $lid, $title, $date, $submitter, $message, $resume, $tele, $email, $r_usid, $company)=$xoopsDB->fetchRow($result)) {
		...
		if ( in_array($member_id, $comp_users) ) {
			$xoopsTpl->assign('del_reply', "<a href=\"delreply.php?r_lid=".addslashes($r_lid)."\"><img src=\"images/del.gif\" border=0 alt=\""._JOBS_DELETE."\" /></a>");
				}
			}
