...
function jobs_show($options)
{
	...
	$result = $xoopsDB->query("SELECT lid, cid, title, status, expire, type, company, desctext, requirements, tel, price, typeprice, contactinfo, date, email, submitter, usid, town, state, valid, photo, view FROM ".$xoopsDB->prefix("".$blockdirname."_listing")." WHERE valid='Yes' and status!='1' $cat_perms ORDER BY ".$options[0]." DESC",$options[1],0);
