...
$block['scroll'] = "<marquee scrollamount=\"2\" behavior=\"scroll\" direction=\"up\" onMouseOver=\"marqid.stop()\" onMouseOut=\"marqid.start()\">";
	...
	$result = $xoopsDB -> query ("SELECT c.comp_id, c.comp_name, c.comp_img, l.company FROM ".$xoopsDB->prefix("jobs_companies")." AS c LEFT OUTER JOIN ".$xoopsDB->prefix("jobs_listing")." AS l on c.comp_name = l.company WHERE l.valid = 'Yes' and l.status!='1' $cat_perms GROUP BY c.comp_img");
