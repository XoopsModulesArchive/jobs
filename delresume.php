...
include XOOPS_ROOT_PATH.'/header.php';
	$result = $xoopsDB->query("select usid, resume FROM ".$xoopsDB->prefix("jobs_resume")." where lid=".mysql_real_escape_string($lid)."");
	...
	$ok = !isset($_REQUEST['ok'])? NULL : $_REQUEST['ok'];
		...
		if ($usid == $calusern) {
			if($ok==1) {
			    if($resume=='created') {
			    $xoopsDB->queryf("delete from ".$xoopsDB->prefix("jobs_created_resumes")." where lid=".mysql_real_escape_string($lid)."");
			      } else {
			    $xoopsDB->queryf("delete from ".$xoopsDB->prefix("jobs_resume")." where lid=".mysql_real_escape_string($lid)."");
			   }
