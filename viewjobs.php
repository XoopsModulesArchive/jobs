...
$lid = intval($_GET['lid']);
	...
	$result=$xoopsDB->query("select lid, cid, title, status, expire, type, company, desctext, requirements, tel, price, typeprice, contactinfo, contactinfo1, contactinfo2, date, email, submitter, usid, town, state, valid, photo, view  FROM ".$xoopsDB->prefix("jobs_listing")." WHERE lid = ".mysql_real_escape_string($lid)." AND status!='1'");
	...
	$result1 = $xoopsDB->query("select cid from ".$xoopsDB->prefix("jobs_listing")." where  lid=".mysql_real_escape_string($lid)."");
	list($cid) = $xoopsDB->fetchRow($result1);

	$result2 = $xoopsDB->query("select cid, pid, title from ".$xoopsDB->prefix("jobs_categories")." where  cid=".mysql_real_escape_string($cid)."");
	...
	list($trows) = $xoopsDB->fetchRow($xoopsDB->query("select COUNT(*) FROM ".$xoopsDB->prefix("jobs_listing")." where valid='Yes' AND cid=".mysql_real_escape_string($cid).""));
    ...
    if ($recordexist) {
		...
		if ($viewcount_judge == true ){
			$xoopsDB->queryF("UPDATE ".$xoopsDB->prefix("jobs_listing")." SET view=view+1 WHERE lid = ".mysql_real_escape_string($lid)."");
		}
		...
		$imprD = "<a href=\"print.php?op=Jprint&amp;lid=".addslashes($lid)."\" target=\"_blank\"><img src=\"images/print.gif\" border=\"0\" alt=\""._JOBS_PRINT."\" width=\"15\" height=\"11\" /></a>&nbsp;";
	...
	$result4=$xoopsDB->query("select comp_id, comp_name, comp_img, comp_usid, comp_user1, comp_user2, comp_user1_contact, comp_user2_contact FROM ".$xoopsDB->prefix("jobs_companies")." where comp_id=". mysql_real_escape_string($comp_id) ." and ". mysql_real_escape_string($usid) ." IN (comp_usid, comp_user1, comp_user2)");
	...
	$xoopsTpl->assign('admin', "<a href=\"admin/index.php?op=ModJob&amp;lid=".addslashes($lid)."\"><img src=\"images/modif.gif\" border=0 alt=\""._JOBS_MODADMIN."\" /></a>");
	}
	}
	...
	$result8 = $xoopsDB->query("select title from ".$xoopsDB->prefix("jobs_categories")." where cid=".mysql_real_escape_string($cid)."");
	...
	$xoopsTpl->assign('friend', "<a href=\"../$mydirname/sendfriend.php?op=SendJob&amp;lid=".addslashes($lid)."\"><img src=\"../$mydirname/images/friend.gif\" border=\"0\" alt=\""._JOBS_SENDTOFRIEND."\" width=\"15\" height=\"11\" /></a>");
