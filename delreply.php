...
include XOOPS_ROOT_PATH."/header.php";
	...
	$result = $xoopsDB->query("select lid, r_usid FROM ".$xoopsDB->prefix("jobs_replies")." where r_lid=".mysql_real_escape_string($r_lid)."");
	...
	if ($xoopsUser) {
		...
		if ( in_array($member_id, $comp_users) ) {
			...
			if($ok==1) {
			    $xoopsDB->queryf("delete from ".$xoopsDB->prefix("jobs_replies")." where r_lid=".mysql_real_escape_string($r_lid)."");
