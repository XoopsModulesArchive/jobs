 ...
 //   $istheirs = false;
	...
	if ($xoopsModuleConfig['jobs_moderate_resume'] == '1') {
		...
		if($res_propo == 0) {
					...
					$xoopsTpl->assign('confirm_resume', _JOBS_RES_THEREIS." $res_propo  "._JOBS_RES_WAIT."<br /><a href=\"admin/index.php\">"._JOBS_RES_SEEIT."</a>");
				}
			}
		}
	}
	...
	$img = "<a href=\"jobscat.php?cid=".$myrow['cid']."\"><img src='".XOOPS_URL."/modules/$mydirname/images/cat/".$cat_img."' align='middle' alt='' /></a>";
	} else {
		$img = '';
	}
	...
	if(in_array($myrow['cid'], $categories)) {
		$arr=$mytree->getFirstChild($myrow['cid'], "".$xoopsModuleConfig["".$mydirname."_cat_sortorder"]."");
		$space = 0;
		$chcount = 0;
		$subcategories = '';
	if ($xoopsModuleConfig["".$mydirname."_display_subcat"] == '1') {
		foreach($arr as $ele) {
			if(in_array($ele['cid'], $categories)) {
				...
				$subcategories .= "<a href=\"".XOOPS_URL."/modules/$mydirname/jobscat.php?cid=".$ele['cid']."\">".$chtitle."</a>";
				$space++;
				$chcount++;
				$content .= $ele['title'].' ';
			}
	...
	list($ann) = $xoopsDB->fetchRow($xoopsDB->query("select  COUNT(*)  FROM ".$xoopsDB->prefix("jobs_listing")." WHERE valid='Yes'$cat_perms"));
	...
	$cat_perms .= ' AND cid IN ('.implode(',', $categories).') ';
		...
		$result=$xoopsDB->query("select lid, title, status, expire, type, company, price, typeprice, date, town, state, valid, photo, view FROM ".$xoopsDB->prefix("jobs_listing")." WHERE valid='Yes' AND status!='1' $cat_perms ORDER BY date DESC LIMIT ".$xoopsModuleConfig['jobs_new_jobs_count']."");
	if ($result) {
	...
	$show_company = false;
		...
		$useroffset = "";
			...
			if ($xoopsUser) {
				if ($xoopsUser->isAdmin()) {
					$a_item['admin'] = "<a href='admin/index.php?op=ModJob&amp;lid=".addslashes($lid)."'><img src='images/modif.gif' border=0 alt=\""._JOBS_MODADMIN."\" /></a>";
				}
			}
	...
	if ($photo) {
	$a_item['photo'] = "<a href=\"javascript:CLA('display-image.php?lid=".addslashes($lid)."')\"><img src=\"images/photo.gif\" border=0 width=15 height=11 alt='"._JOBS_IMGPISP."' /></a>";
	}
	$a_item['views'] = $vu;
	$rank++;
