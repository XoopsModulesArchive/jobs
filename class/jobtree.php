...
class JobTree
{
	...
	//$preset_id is used to specify a preselected item
	//set $none to 1 to add a option with value 0
function makeMySearchSelBox($title,$order="",$preset_id=0, $none=0, $sel_name="", $onchange="", $by_cat=0)
	{
	global $mydirname;
		if ( $sel_name == "" ) {
			$sel_name = $this->id;
...
function makeStateBox($title,$order="",$preset_name="", $none=0, $sel_name="", $onchange="", $by_state=0, $issearch=1)
	{
	global $mydirname;
		if ( $sel_name == "" ) {
			$sel_name = $this->id;
	...
	function makeAdminStateBox($title,$order="",$preset_name="", $none=0, $sel_name="", $onchange="", $by_state=0, $issearch=0)
	{
	global $mydirname;
		if ( $sel_name == "" ) {
			$sel_name = $this->id;
	...
	function makeJobSelBox($title,$order="",$preset_id=0, $none=0, $sel_name="", $onchange=""){
		...
		while ( list($catid, $name, $ordre) = $xoopsDB->fetchRow($result2) ) {
			...
			echo "<p><a href=\"category.php?op=NewCat&amp;cid=$catid\"><img src=\"".XOOPS_URL."/modules/jobs/images/plus.gif\" border=0 width=10 height=10 alt=\""._AM_JOBS_ADDSUBCAT."\"></a>&nbsp;<a href=\"category.php?op=ModCat&amp;cid=$catid\" title=\""._AM_JOBS_MODIFCAT."\">$name</a> ";
				if ($xoopsModuleConfig['jobs_cat_sortorder'] == "ordre") { echo "($ordre)"; }				
				echo "<br />\n";
			$arr = $this->getChildTreeMapArray($catid, $order);
			foreach ( $arr as $option ) {
				$option['prefix'] = str_replace("."," --->",$option['prefix']);
				$catpath = $option['prefix']."&nbsp;<a href=\"category.php?op=NewCat&amp;cid=".$option[$this->id]."\"><img src=\"".XOOPS_URL."/modules/jobs/images/plus.gif\" border=0 width=10 height=10 alt=\""._AM_JOBS_ADDSUBCAT."\"></a>&nbsp;<a href=\"category.php?op=ModCat&amp;cid=".$option[$this->id]."\" title=\""._AM_JOBS_MODIFCAT."\">".$myts->htmlSpecialChars($option[$title]);
				$ordreS = $option['ordre'];
				echo "$catpath</a> ";
				if ($xoopsModuleConfig['jobs_cat_sortorder'] == "ordre") { echo "($ordreS)"; }				
				echo "<br />\n";
	...
	$mydirname = basename( dirname( dirname( __FILE__ ) ) ) ;
		...
		while ( list($catid, $name, $ordre) = $xoopsDB->fetchRow($result) ) {
			...
			echo "<p><a href=\"category.php?op=NewResCat&amp;cid=$catid\"><img src=\"".XOOPS_URL."/modules/jobs/images/plus.gif\" border=0 width=10 height=10 alt=\""._AM_JOBS_ADDSUBCAT."\"></a>&nbsp;<a href=\"category.php?op=ModResCat&amp;cid=$catid\" title=\""._AM_JOBS_MODIFCAT."\">".$name."</a> ";
				if ($xoopsModuleConfig['jobs_cat_sortorder'] == "ordre") { echo "($ordre)"; }				
				echo "<br />\n";
			$arr = $this->getChildTreeMapArray($catid, $order);
			foreach ( $arr as $option ) {
				$option['prefix'] = str_replace("."," --->",$option['prefix']);
				$catpath = $option['prefix']."&nbsp;<a href=\"category.php?op=NewResCat&amp;cid=".$option[$this->id]."\"><img src=\"".XOOPS_URL."/modules/jobs/images/plus.gif\" border=0 width=10 height=10 alt=\""._AM_JOBS_ADDSUBCAT."\"></a>&nbsp;<a href=\"category.php?op=ModResCat&amp;cid=".$option[$this->id]."\" title=\""._AM_JOBS_MODIFCAT."\">".$myts->htmlSpecialChars($option[$title]);
				$ordreS = $option['ordre'];
				echo "$catpath</a> ";
				if ($xoopsModuleConfig['jobs_cat_sortorder'] == "ordre") { echo "($ordreS)"; }				
				echo "<br />\n";
