...
class ResTree
{
	...
	//$preset_id is used to specify a preselected item
	//set $none to 1 to add a option with value 0
	function resume_makeMySearchSelBox($title,$order="",$preset_id=0, $none=0, $sel_name="", $onchange="", $by_cat=0)
	{
	global $mydirname;
		if ( $sel_name == "" ) {
			$sel_name = $this->id;
...
function resume_makeStateSelBox($title,$order="",$preset_id=0, $none=0, $sel_name="", $onchange="")
	...
	function resume_makeStateBox($title,$order="",$preset_name="", $none=0, $sel_name="", $onchange="", $by_state=0, $issearch=0)
	{
	global $mydirname;
		if ( $sel_name == "" ) {
			$sel_name = $this->id;
	...
	$mydirname = basename( dirname( dirname( __FILE__ ) ) ) ;
		...
		while ( list($catid, $name, $ordre) = $xoopsDB->fetchRow($result) ) {
			...
			foreach ( $arr as $option ) {
				$option['prefix'] = str_replace("."," --->",$option['prefix']);
				$catpath = $option['prefix']."&nbsp;<a href=\"category.php?op=NewResCat&amp;cid=".$option[$this->id]."\"><img src=\"".XOOPS_URL."/modules/$mydirname/images/plus.gif\" border=0 width=10 height=10 alt=\""._AM_JOBS_ADDSUBCAT."\"></a>&nbsp;<a href=\"category.php?op=ModResCat&amp;cid=".$option[$this->id]."\" title=\""._AM_JOBS_MODIFCAT."\">".$myts->htmlSpecialChars($option[$title]);
				$ordreS = $option['ordre'];
				echo "$catpath</a> ";
