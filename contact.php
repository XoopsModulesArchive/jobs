...
if (!$gperm_handler->checkRight("".$mydirname."_view", $perm_itemid, $groups, $module_id)) {
	...
	if ( ! $xoopsGTicket->check( true , 'token' ) ) {
		redirect_header(XOOPS_URL."/modules/$mydirname/viewjobs.php?lid=".addslashes($id)."", 3,$xoopsGTicket->getErrors());
	}
...
//	}
	...
	$result = $xoopsDB->query("select lid, email, submitter, title, type, company, desctext, requirements, usid FROM  ".$xoopsDB->prefix("jobs_listing")." WHERE lid = ".mysql_real_escape_string($id)."");
...
//If no access
if (!$gperm_handler->checkRight("".$mydirname."_view", $perm_itemid, $groups, $module_id)) {
    redirect_header(XOOPS_URL."/index.php", 3, _NOPERM);    exit();
}
	...
	{
	echo "<option value=\"".XOOPS_URL."/modules/$mydirname/viewresume.php?lid=$rlid&amp;unlock=$private\">".$title."</option>";
	}	
	echo "</select></td></tr>";
	}
...
//	echo "</td></tr>";
	...
	echo "<input type=\"hidden\" name=\"submit\" value=\"1\" />";
	echo "<p><input type=\"submit\" value=\""._JOBS_SENDFR."\" /></p>".$GLOBALS['xoopsGTicket']->getTicketHtml( __LINE__ , 1800 , 'token')."
	</form>";
	}
	}
	echo "</td></tr></table>";
