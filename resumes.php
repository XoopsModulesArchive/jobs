...
$statetree = new ResTree($xoopsDB->prefix("jobs_region"),"rid","pid");
	...
	$your_res=$xoopsDB->query("select lid,title,usid FROM ".$xoopsDB->prefix("jobs_resume")." WHERE usid =".mysql_real_escape_string($member_usid)."");
		...
		$member_usid = $xoopsUser->getVar("uid", "E");
			if ($xoopsUser->isAdmin()) {
				...
				if($propo == 0) {
					...
					$xoopsTpl->assign('confirm_resume', _JOBS_THEREIS." $propo  "._JOBS_WAIT."<br /><a href=\"admin/index.php\">"._JOBS_SEEIT."</a>");
				}
			}
		}
	}
	...
	if ($myrow['img'] && $myrow['img'] != 'http://') {
		...
		$img = "<a href=\"resumecat.php?cid=".$myrow['cid']."\"><img src='".XOOPS_URL."/modules/$mydirname/images/cat/".$cat_img."' align='middle' alt='' /></a>";
	} else {
		$img = '';
	}
	...
	list($ann) = $xoopsDB->fetchRow($xoopsDB->query("select  COUNT(*)  FROM ".$xoopsDB->prefix("jobs_resume")." WHERE valid='Yes'$cat_perms"));
//	list($catt) = $xoopsDB->fetchRow($xoopsDB->query("select  COUNT(*)  FROM ".$xoopsDB->prefix("jobs_res_categories")." $cat_perms2"));
	...
	$result=$xoopsDB->query("select lid, name, title, status, exp, expire, private, salary, typeprice, date, usid, town, state, valid, view FROM ".$xoopsDB->prefix("jobs_resume")." WHERE valid='Yes' $cat_perms ORDER BY date DESC LIMIT ".$xoopsModuleConfig['jobs_new_jobs_count']."");
	if ($result){
	...
	while(list($lid, $name, $title, $status, $exp, $expire, $private, $salary, $typeprice, $date, $usid, $town, $state, $valid, $vu)=$xoopsDB->fetchRow($result)) {
		...
		if ($xoopsUser) {
		if ($xoopsUser->isAdmin()) {
			$a_item['admin'] = "<a href='admin/modresume.php?lid=".addslashes($lid)."'><img src='images/modif.gif' border=0 alt='"._JOBS_MODRESADMIN."' /></a>";
		}
		}
