...
require_once( XOOPS_ROOT_PATH."/modules/$mydirname/include/gtickets.php" ) ;
	...
	redirect_header(XOOPS_URL."/modules/$mydirname/viewresume.php?lid=".addslashes($id)."", 3,$xoopsGTicket->getErrors());
	}
...
//	}
//	}
	$result = $xoopsDB->query("select name, email, submitter, title FROM  ".$xoopsDB->prefix("jobs_resume")." WHERE lid = ".mysql_real_escape_string($id)."");
	...
	echo "<input type=\"hidden\" name=\"submit\" value=\"1\" />";
		...
		$result1 = $xoopsDB->query("select lid, cid, title, company, email, submitter FROM  ".$xoopsDB->prefix("jobs_listing")." WHERE email = '$idde' and lid = ".mysql_real_escape_string($lid)."");
	...
	while(list($lid, $title, $date, $email) = $xoopsDB->fetchRow($dropdown))
		{
		echo "<option value=\"".XOOPS_URL."/modules/".$mydirname."/viewjobs.php?lid=$lid\">".$title."</option>";
		}	
			echo "</select></td></tr>";
		}
...
//	}
	...
	<br />
      <p><input type=\"submit\" name=\"submit\" value=\""._JOBS_SENDFR."\" /></p>".$GLOBALS['xoopsGTicket']->getTicketHtml( __LINE__ , 1800 , 'token')."
	</form>";
}
	echo "</td></tr></table>";
